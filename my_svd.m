...
%   The eigenvalues of the R_x=X'X matrix are calculated and then the number of 
%   eigenvectors that must be computed is determined as the smallest
%   value that satisfies the expression (8). Finally transform matrices are
%   computed
    R_x = X'*X;
    l = eig(R_x);
    l = sort(l,'descend');
    ...
    c = sqrt(l(1:p));
    for j=1:p
        u_r = null(R_x-l(j)*eye(size(R_x))); %find solution for (A-lambda*I)v=0
        U_r(:,j) = u_r;
        U_l(:,j) = (X*u_r)./c(j);
    end
end
